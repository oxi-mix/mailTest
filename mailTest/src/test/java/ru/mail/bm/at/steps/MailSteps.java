...
/*	
	@Step("ввод данных для логина portal page")
	public void dataInputMail(String loginMailbox, String passwordMailbox) {
		try {
			common.switchFrame(mailPage.ifamePopup);
			System.out.println(common.geIsDisplayedLocator(mailPage.loginMailAccount));
			common.locatorClick(mailPage.loginMailAccount);
			
			System.out.println(common.geIsDisplayedLocator(mailPage.loginMailAccount));
			common.inputValueType(mailPage.loginMailAccount, loginMailbox);
			log.info("login:" + loginMailbox);
			common.inputValueType(mailPage.passwordMailAccount, passwordMailbox);
			log.info("password:" + config.getString(ATConfiguration.pass));
		}
		catch (Exception ex){
			log.debug("",ex);
